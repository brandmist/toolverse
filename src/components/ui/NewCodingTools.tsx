    ...
    <div className="flex flex-col gap-4 h-full bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      <div className="flex gap-3 items-center">
        <div className="flex gap-2">
          <button onClick={() => setMode('minify')} className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${mode === 'minify' ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted hover:text-primary'}`}>Minify</button>
          <button onClick={() => setMode('prettify')} className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${mode === 'prettify' ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted hover:text-primary'}`}>Prettify</button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-grow min-h-[350px]">
        <div className="flex flex-col">
          <label className="text-sm text-primary mb-2">Input JSON ({input.length} bytes)</label>
          <textarea
        ...
        <div className="flex flex-col">
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm text-primary">Output {output && `(${output.length} bytes)`}</label>
  ...
  return (
    <div className="flex flex-col gap-4 h-full bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      <div className="flex gap-4 text-sm flex-wrap">
        ...
        {stats.added === 0 && stats.removed === 0 && stats.changed === 0 && <span className="text-muted">No differences found</span>}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="text-sm text-primary mb-2 block">Original Code</label>
          <textarea className="w-full h-52 p-4 bg-white border border-border border-border rounded-xl resize-none text-primary font-mono text-sm focus:outline-none focus:ring-1 focus:ring-text-primary" value={left} onChange={e => setLeft(e.target.value)} />
        </div>
        <div>
          <label className="text-sm text-primary mb-2 block">Modified Code</label>
          <textarea className="w-full h-52 p-4 bg-white border border-border border-border rounded-xl resize-none text-primary font-mono text-sm focus:outline-none focus:ring-1 focus:ring-text-primary" value={right} onChange={e => setRight(e.target.value)} />
        </div>
      </div>

      <div className="bg-white border border-border border-border rounded-xl overflow-auto max-h-80 flex-grow">
        <div className="grid grid-cols-[auto_1fr_auto_1fr] text-xs font-mono">
          <div className="bg-white border border-border px-2 py-1.5 text-muted border-b border-border col-span-2 font-semibold">ORIGINAL</div>
          <div className="bg-white border border-border px-2 py-1.5 text-muted border-b border-border col-span-2 font-semibold">MODIFIED</div>
          {diff.map((row, i) => {
            ...
            const textMap = { same: 'text-muted', changed: 'text-warning', added: 'text-primary', removed: 'text-danger' }
            const textMapR = { same: 'text-muted', changed: 'text-warning', added: 'text-success', removed: 'text-primary' }
            const prefix = { same: ' ', changed: '~', added: '+', removed: '-' }
            const prefixColor = { same: 'text-primary', changed: 'text-warning', added: 'text-success', removed: 'text-danger' }
            return (
              <>
                <div key={`lp-${i}`} className={`px-2 py-0.5 ${bgMap[row.type]} ${prefixColor[row.type]}`}>{prefix[row.type]}</div>
                <div key={`l-${i}`} className={`px-3 py-0.5 ${bgMap[row.type]} ${textMap[row.type]} whitespace-pre border-r border-border`}>{row.left ?? ''}</div>
    ...
    <div className="flex flex-col md:flex-row gap-8 h-full">
      ...
      <div className="flex-[1.2] flex flex-col gap-6 bg-white border border-border rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-primary">Radius Settings</h3>
          <button onClick={generateRandom} className="p-3 text-muted hover:text-primary hover:bg-surface-hover rounded-xl transition-colors" title="Randomize Blob">
            <Shuffle className="w-5 h-5" />
          </button>
        </div>

        <div>
          <label className="text-[13px] font-semibold text-secondary block mb-2">Mode</label>
          <div className="flex bg-surface-hover p-1 rounded-xl">
            <button onClick={() => setMode('simple')} className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'simple' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-secondary'}`}>Standard</button>
            <button onClick={() => setMode('advanced')} className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'advanced' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-secondary'}`}>Fancy (8-Point)</button>
          </div>
        </div>
        ...
        <div className="space-y-6 mt-2">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-5">
              <h4 className="text-[12px] font-bold text-subtle uppercase tracking-wider">{mode === 'advanced' ? 'Horizontal (X)' : 'Corners'}</h4>
              {[
                { label: 'Top Left', k: 'tl' },
                ...
                <div key={ctrl.k}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-[13px] font-medium text-muted">{vals[ctrl.k as keyof typeof vals]}%</span>
                  </div>
            ...
            {mode === 'advanced' && (
              <div className="space-y-5">
                <h4 className="text-[12px] font-bold text-subtle uppercase tracking-wider">Vertical (Y)</h4>
                {[
      ...
      <div className="flex-[1] flex flex-col gap-6">
        <div className="flex-1 bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] rounded-2xl border border-border min-h-[350px] flex items-center justify-center p-8 overflow-hidden relative radial-bg">
...
export function JsonToTs() {
  ...
  return (
    <div className="flex flex-col gap-4 h-full bg-white border border-border rounded-2xl p-6 shadow-sm">
      <div className="flex gap-3 items-center">
        {error && <span className="text-danger text-sm">{error}</span>}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-grow min-h-[350px]">
        <div className="flex flex-col">
          <label className="text-sm text-primary mb-2">Input JSON</label>
          <textarea
            className="flex-grow p-4 bg-white border border-border rounded-2xl resize-none text-primary font-mono text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
            value={input}
            onChange={e => setInput(e.target.value)}
          />
        </div>
        <div className="flex flex-col relative group">
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm text-primary">TypeScript Interfaces</label>
            {output && <button onClick={copy} className="text-xs text-muted hover:text-primary flex items-center gap-1">{copied ? <Check className="w-3 h-3 text-[#34D399]" /> : <Copy className="w-3 h-3" />} Copy</button>}
          </div>
          <textarea
            readOnly
            className="flex-grow p-4 bg-surface border border-border rounded-xl resize-none text-primary font-mono text-sm"
            value={output}
...
export function CsvJsonConverter() {
  ...
  return (
    <div className="flex flex-col gap-4 h-full bg-white border border-border rounded-2xl p-6 shadow-sm">
      <div className="flex gap-3 items-center">
        <div className="flex bg-surface-hover p-1 rounded-xl">
          <button onClick={() => setMode('csv-to-json')} className={`px-4 py-1.5 rounded-md text-[13px] font-semibold transition-all ${mode === 'csv-to-json' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-primary'}`}>CSV to JSON</button>
          <button onClick={() => setMode('json-to-csv')} className={`px-4 py-1.5 rounded-md text-[13px] font-semibold transition-all ${mode === 'json-to-csv' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-primary'}`}>JSON to CSV</button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-grow min-h-[350px]">
        ...
        <textarea
          readOnly
          className="flex-grow p-4 bg-surface border border-border rounded-xl resize-none text-success font-mono text-sm"
          value={output}
