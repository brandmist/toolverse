    ...
    <div className="relative w-full">
      <div
        ...
        className={`relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl transition-all cursor-pointer overflow-hidden
          ${isDragging 
            ? 'bg-surface border border-border border-border text-text-primary' 
            : 'border-white/20 hover:bg-white/5 text-text-muted hover:text-text-primary hover:border-border'
          }`}
      >
        <input 
          type="file" 
        ...
        <p className="text-sm text-text-muted mt-2 select-none pointer-events-none">{subtitle}</p>
      </div>

      {/* FileErrorToast */}
      <div 
        className={`absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-3 bg-card border border-border border-border text-danger rounded-lg shadow-xl shadow-rose-500/10 transition-all duration-300 ease-out pointer-events-auto
