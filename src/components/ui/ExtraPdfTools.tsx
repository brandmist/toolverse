    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      {!file ? (
        ...
        >
          ...
          <p className="text-text-primary font-medium mb-1">Click to upload PDF to extract to Table</p>
          <p className="text-text-muted text-xs">Parses rows and columns to generate clean spreadsheets (.csv / .xlsx)</p>
        </div>
      ) : (
        <div className="space-y-4 w-full">
          <div className="flex items-center justify-between border-b border-white/15 pb-4">
            <div>
              <h3 className="font-semibold text-text-primary truncate max-w-sm">{file.name}</h3>
              <p className="text-xs text-text-muted">Heuristically parsing table cells</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setFormat('csv')} className={`px-3 py-1 rounded text-xs font-bold ${format === 'csv' ? 'bg-card border border-border text-text-primary' : 'bg-card border border-border text-text-muted'}`}>CSV</button>
              <button onClick={() => setFormat('xlsx')} className={`px-3 py-1 rounded text-xs font-bold ${format === 'xlsx' ? 'bg-card border border-border text-text-primary' : 'bg-card border border-border text-text-muted'}`}>Excel (XLSX)</button>
            </div>
          </div>
          ...
          {preview.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider">Spreadsheet Preview (First 10 Rows)</h4>
              <div className="overflow-x-auto border border-white/10 rounded-xl bg-surface border border-border">
                <table className="min-w-full divide-y divide-white/10 text-left text-xs text-text-primary">
                  <tbody className="divide-y divide-white/5 font-mono">
                    {preview.map((row, i) => (
                      <tr key={i} className="hover:bg-white/5">
                        {row.map((cell, j) => (
                          <td key={j} className="px-4 py-2 border-r border-white/5 whitespace-nowrap">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="flex gap-2">
            <Button onClick={() => handleConvert(file)} className="flex-1 bg-card border border-border hover:bg-card-hover text-text-primary text-sm" disabled={processing}>
              Re-Convert & Download
            </Button>
            <Button variant="outline" className="border-white/10 text-text-primary text-sm" onClick={() => { setFile(null); setPreview([]) }}>
              Clear
            </Button>
          </div>
        </div>
    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      {!file ? (
        ...
        >
          ...
          <p className="text-text-primary font-medium mb-1">Click to upload PDF to Translate</p>
          <p className="text-text-muted text-xs">Extracts text and generates a fully translated version of the layout sheets</p>
        </div>
      ) : (
        <div className="space-y-4 max-w-md mx-auto w-full">
          ...
          <div>
            <label className="text-xs text-text-muted block mb-1">Select Target Translation Language</label>
            <select 
              value={targetLang} 
              onChange={e => setTargetLang(e.target.value)}
              className="w-full bg-card border border-border border-white/10 rounded-xl px-4 py-2.5 text-text-primary text-sm"
            >
              {languages.map(l => (
          ...
          <div className="flex gap-2">
            <Button onClick={handleTranslate} className="flex-1 bg-card border border-border hover:bg-card-hover text-text-primary font-semibold text-sm" disabled={processing}>
              Translate PDF
            </Button>
            <Button variant="outline" className="border-white/10 text-text-primary text-sm" onClick={() => { setFile(null); setError('') }}>
              Clear
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        ...
        <p className="text-text-primary font-medium mb-1">Click to upload PDF to convert to EPUB</p>
        <p className="text-text-muted text-xs">Packs structural page contents into fully compliant reflowable .epub format</p>
      </div>
...
export function MobiToPdf() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        <BookOpen className="w-12 h-12 text-danger mb-4" />
        <p className="text-text-primary font-medium mb-1">Click to upload MOBI to convert to PDF</p>
        <p className="text-text-muted text-xs">Unpacks Mobipocket eBooks and renders textual streams to PDF format</p>
      </div>
...
export function PdfToMobi() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        <BookOpen className="w-12 h-12 text-danger mb-4" />
        <p className="text-text-primary font-medium mb-1">Click to upload PDF to convert to MOBI</p>
        <p className="text-text-muted text-xs">Packs page textual layouts into a standard Mobipocket eBook wrapper</p>
      </div>
...
export function Azw3ToPdf() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        <BookOpen className="w-12 h-12 text-danger mb-4" />
        <p className="text-text-primary font-medium mb-1">Click to upload AZW3 to convert to PDF</p>
        <p className="text-text-muted text-xs">Decodes Kindle KF8 layout structures and layouts text nodes to PDF</p>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        ...
        <p className="text-text-primary font-medium mb-1">Click to upload PowerPoint (PPTX) to convert to PDF</p>
        <p className="text-text-muted text-xs">Unzips presentations and lays out text placeholders to landscape PDF sheets</p>
      </div>
    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      <form onSubmit={handleConvert} className="space-y-4 max-w-lg mx-auto w-full">
        <div className="text-center">
          ...
          <h3 className="text-text-primary font-medium mb-1">Convert URL to PDF</h3>
          <p className="text-text-muted text-xs">Enter a website URL link to scrape main contents and compile into a PDF layout</p>
        </div>
        ...
        <div className="flex gap-2">
          <input 
            ...
            required
            className="flex-grow bg-card border border-border border-white/10 rounded-xl px-4 py-2.5 text-text-primary text-sm"
          />
          <Button type="submit" className="bg-card border border-border hover:bg-card-hover text-text-primary font-semibold text-sm">
            Generate PDF
          </Button>
        </div>

        {error && <div className="text-xs text-danger bg-surface border border-border border-border rounded-lg p-3 text-center">{error}</div>}
      </form>
    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      {!file ? (
        ...
        >
          ...
          <p className="text-text-primary font-medium mb-1">Click to upload PDF to remove Watermarks</p>
          <p className="text-text-muted text-xs">Stamps opaque mask structures to white out layout watermarks locally</p>
        </div>
      ) : (
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[320px] flex flex-col justify-between">
            <div className="space-y-4">
              <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider">Watermark Stripper</h4>
              <div>
                <label className="text-xs text-text-muted block mb-1">Watermark Keyword Target</label>
                <input type="text" value={matchText} onChange={e => setMatchText(e.target.value)} className="w-full bg-card border border-border border-white/10 rounded-lg p-2 text-text-primary text-xs font-mono" />
              </div>

              <div className="space-y-1">
                <button 
                  onClick={() => setWhiteAreas([...whiteAreas, { x: 25, y: 25, w: 50, h: 10 }])}
                  className="w-full py-1.5 bg-card border border-border hover:bg-slate-950 text-text-primary border border-white/10 rounded-lg text-xs font-semibold"
                >
            ...
            <div className="space-y-2 pt-4">
              <Button onClick={handleStrip} className="w-full bg-card border border-border hover:bg-card-hover text-text-primary text-sm" disabled={processing}>
                Remove Watermarks & Save
              </Button>
              <Button variant="outline" className="w-full border-white/10 text-text-primary text-sm" onClick={() => { setFile(null); setWhiteAreas([]) }}>
                Clear
              </Button>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      <div className="space-y-4 max-w-xl mx-auto w-full">
        <div>
          <label className="text-xs text-text-muted block mb-1">Document Title</label>
          <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-card border border-border border-white/10 rounded-xl px-4 py-2 text-text-primary font-semibold text-sm" />
        </div>
        <div>
          <label className="text-xs text-text-muted block mb-1">Subtitle / Author Description</label>
          <input type="text" value={subtitle} onChange={e => setSubtitle(e.target.value)} className="w-full bg-card border border-border border-white/10 rounded-xl px-4 py-2 text-text-primary text-sm" />
        </div>
        <div>
          <label className="text-xs text-text-muted block mb-1">Document Body Text</label>
          <textarea 
            rows={8}
            value={content} 
            onChange={e => setContent(e.target.value)}
            className="w-full bg-card border border-border border-white/10 rounded-xl p-4 text-text-primary text-sm font-mono leading-relaxed"
          />
        </div>

        <div className="flex justify-between items-center gap-4 bg-surface border border-border p-4 rounded-xl border border-white/5">
          <div>
            <label className="text-xs text-text-muted block mb-1">Page Sheet Configuration</label>
            <div className="flex gap-2">
              <button onClick={() => setPageSize('A4')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${pageSize === 'A4' ? 'bg-card border border-border text-text-primary' : 'bg-card border border-border text-text-muted'}`}>A4 Size</button>
              <button onClick={() => setPageSize('Letter')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${pageSize === 'Letter' ? 'bg-card border border-border text-text-primary' : 'bg-card border border-border text-text-muted'}`}>Letter</button>
            </div>
          </div>
          <Button onClick={handleCreate} className="bg-card border border-border hover:bg-card-hover text-text-primary font-semibold text-sm px-6 h-10" disabled={processing}>
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
      ...
      >
        ...
        <p className="text-text-primary font-medium mb-1">Click to upload {targetType.toUpperCase()} file</p>
        <p className="text-text-muted text-xs">Instantly formats and renders your image into a standard page-compatible PDF</p>
      </div>
