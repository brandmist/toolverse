...
export const ToolCard: React.FC<ToolCardProps> = ({ tool, index = 0 }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full bg-white border border-border rounded-2xl p-5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:border-border-hover transition-all duration-300">
        
        {/* Header: Icon + Title */}
        <div className="flex items-start gap-4 mb-3">
          <div className="w-12 h-12 rounded-xl bg-surface border border-border flex items-center justify-center text-secondary shrink-0 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all duration-300">
            <Icon name={tool.icon} className="w-6 h-6" />
          </div>
          <div className="flex-1 pt-0.5 min-w-0">
            <div className="flex items-start justify-between gap-3 mb-1">
              <h3 className="text-[16px] font-bold text-primary group-hover:text-black leading-tight line-clamp-3 pr-1">
                {tool.name}
              </h3>
              {/* Badges */}
            ...
            <p className="text-[12px] font-semibold text-muted uppercase tracking-wider truncate">{category?.name}</p>
          </div>
        </div>

        {/* Description */}
        <p className="text-[14px] text-muted leading-relaxed line-clamp-3 mb-5 flex-1">
          {tool.description}
        </p>
