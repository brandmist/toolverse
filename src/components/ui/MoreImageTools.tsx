    ...
    <div className="flex flex-col gap-6 h-full bg-white border border-border rounded-2xl p-6 ">
      {!file ? (
        ...
        >
          ...
          <p className="text-primary font-medium mb-1">Click to upload photo to change background</p>
          <p className="text-muted text-xs">Isolates subjects locally in browser and applies custom background styles</p>
        </div>
      ) : (
             ...
             <div className="space-y-4">
               {!removedBgUrl ? (
                 <div className="space-y-2">
                   <p className="text-xs text-muted">Step 1: Isolate subject from original background</p>
                 ...
                 <div className="space-y-4">
                   ...
                   <div>
                     <label className="text-xs text-muted font-semibold uppercase tracking-wider mb-2 block">Background Type</label>
                     <div className="grid grid-cols-2 gap-2">
                       <button onClick={() => setBgType('color')} className={`py-1.5 rounded-xl text-xs font-bold ${bgType === 'color' ? 'bg-white border border-border text-primary' : 'bg-white border border-border text-muted'}`}>Color</button>
                       <button onClick={() => setBgType('image')} className={`py-1.5 rounded-xl text-xs font-bold ${bgType === 'image' ? 'bg-white border border-border text-primary' : 'bg-white border border-border text-muted'}`}>Image</button>
                     </div>
                   </div>
                   
                   {bgType === 'color' ? (
                     <div>
                       <label className="text-xs text-muted font-semibold uppercase tracking-wider mb-2 block">Fill Color</label>
                     ...
                     <div>
                       <label className="text-xs text-muted font-semibold uppercase tracking-wider mb-2 block">Background Image</label>
                       <input 
                         type="file" 
