  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[350px] flex flex-col">
          <h3 className="font-semibold text-text-primary flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-text-primary" />
            AI Generator
          </h3>
          <p className="text-sm text-text-muted">Describe what you want to see, and AI will generate it for free.</p>
          
          <div className="space-y-4 flex-1">
            <div>
              <label className="text-sm text-text-primary block mb-2">Prompt</label>
              <textarea 
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g. A cyberpunk city at night with neon lights..."
                className="w-full bg-surface border border-border border-white/10 rounded-xl p-3 text-sm text-text-primary focus:outline-none focus:border-border min-h-[120px] resize-none"
              />
            </div>
            <div>
              <label className="text-sm text-text-primary block mb-2">Size</label>
              <select 
                value={size} 
                onChange={(e) => setSize(e.target.value)}
                className="w-full bg-surface border border-border border-white/10 rounded-xl p-3 text-sm text-text-primary focus:outline-none focus:border-border"
              >
          ...
          <Button 
            className="w-full bg-card border border-border hover:bg-card-hover text-text-primary mt-auto" 
        ...
        <div className="flex-[2] flex flex-col items-center justify-center p-4 bg-card border border-border rounded-xl border border-white/5 min-h-[400px]">
          {isGenerating ? (
            ...
            <div className="relative group w-full h-full flex items-center justify-center">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-4">
                <Button className="bg-card border border-border hover:bg-card-hover text-text-primary" onClick={handleDownload}>
      ...
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[300px]">
            <h3 className="font-semibold text-text-primary flex items-center gap-2">
              <Scissors className="w-5 h-5 text-text-primary" />
              Background Remover
            </h3>
            ...
            <div className="pt-4 flex flex-col gap-3">
              {!processedUrl ? (
                <Button 
                  className="w-full bg-card border border-border hover:bg-card-hover text-text-primary" 
                ...
                <Button className="w-full bg-card border border-border hover:bg-card-hover text-text-primary" onClick={handleDownload}>
                  <Download className="w-4 h-4 mr-2" /> Download Transparent Image
                </Button>
              )}
              <Button variant="outline" className="w-full border-white/10 hover:bg-white/10 text-text-primary" onClick={handleClear} disabled={isProcessing}>
                Clear
              </Button>
            </div>
          </div>
          <div className="flex-[2] flex items-center justify-center p-4 bg-card border border-border rounded-xl border border-white/5 min-h-[300px] overflow-hidden bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAMUlEQVQ4T2NkYGAQYcAP3uCTZvQAw8gAAQMDwzyUog+S1AwybAAx6oZRMwyDQeQAAwMAXgEHB60tE7EAAAAASUVORK5CYII=')]">
            <img 
      ...
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-[1] flex flex-col gap-4">
             <div className="bg-card border border-border p-2 rounded-xl flex items-center justify-center overflow-hidden border border-white/10 h-[300px]">
               <img src={imageSrc} alt="Preview" className="max-w-full max-h-full object-contain rounded-lg" />
             </div>
             
             {!text && !isProcessing && (
               <Button className="w-full bg-card border border-border hover:bg-card-hover text-text-primary" onClick={processImage}>
                 <ScanText className="w-4 h-4 mr-2" /> Extract Text
               </Button>
             )}
             {isProcessing && (
                <div className="flex items-center justify-center p-3 text-text-primary bg-surface border border-border rounded-xl border border-border gap-2">
          ...
          <div className="flex-[1.5] bg-surface border border-border rounded-xl border border-white/5 p-4 flex flex-col relative min-h-[300px]">
             {text ? (
               <>
                 <textarea 
                   readOnly 
                   className="w-full flex-1 bg-transparent resize-none text-text-primary text-sm focus:outline-none" 
                   value={text} 
                 />
                 <Button size="sm" className="absolute top-4 right-4 bg-card border border-border hover:bg-card-hover text-text-primary border-white/10" onClick={handleCopy}>
          ...
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[300px]">
            ...
            <div className="pt-2">
               <div className="flex justify-between mb-1">
                 <label className="text-sm text-text-primary">Sharpen Intensity</label>
                 <span className="text-sm text-text-muted">{intensity}x</span>
               </div>
               <input 
                 type="range" 
               ...
               />
               <p className="text-xs text-text-muted mt-2">Increase to add edge contrast.</p>
            </div>
            
            <div className="pt-4 flex flex-col gap-3">
              <Button className="w-full bg-card border border-border hover:bg-card-hover text-text-primary" onClick={handleDownload} disabled={!processed}>
                <Download className="w-4 h-4 mr-2" /> Download Result
              </Button>
              <Button variant="outline" className="w-full border-white/10 hover:bg-white/10 text-text-primary" onClick={clearImage}>
                Clear
              </Button>
            </div>
          </div>
    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[300px]">
            <h3 className="font-semibold text-text-primary flex items-center gap-2">
              <Eraser className="w-5 h-5 text-text-primary" />
              Cleanup / Blur Brush
            </h3>
            ...
            <div className="pt-2">
               <div className="flex justify-between mb-1">
                 <label className="text-sm text-text-primary">Brush Size</label>
                 <span className="text-sm text-text-muted">{brushSize}px</span>
               </div>
               <input 
                 type="range" 
            ...
            <div className="pt-4 flex flex-col gap-3">
              ...
              <Button variant="outline" className="w-full border-white/10 hover:bg-white/10 text-text-primary" onClick={resetCanvas}>
                Undo All
              </Button>
          ...
          <div className="flex-[2] flex justify-center bg-card border border-border rounded-xl border border-white/5 overflow-hidden touch-none relative">
            <canvas 
              ref={canvasRef} 
