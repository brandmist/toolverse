  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 h-full bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      <div className="flex-1 space-y-6">
        <div>
          <label className="text-sm text-primary block mb-1">HEX Color</label>
          <div className="flex items-center gap-2">
            ...
            <input type="text" value={hex} onChange={e => setHex(e.target.value)} className="flex-grow bg-white border border-border border-border rounded-xl px-4 py-3 text-primary text-lg font-mono uppercase" />
          </div>
        </div>
        <div>
           <div className="flex justify-between mb-1">
             <span className="text-sm text-primary">Opacity</span>
      ...
      <div className="flex-1 right-panel flex items-center justify-center relative rounded-xl border border-border min-h-[300px]" style={{
         ...
         <div className="z-10 p-4 bg-surface border border-border backdrop-blur-sm border border-border rounded-xl text-center shadow-xl">
            <span className="text-xs text-muted block mb-1 font-semibold uppercase">RGBA Output</span>
...
export function ColorShades() {
  ...
  return (
    <div className="flex flex-col h-full gap-6 bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      <div>
        <label className="text-sm text-primary block mb-1">Base Color</label>
        <div className="flex items-center gap-3 max-w-sm">
          ...
          <input type="text" value={base} onChange={e => setBase(e.target.value)} className="flex-grow bg-white border border-border border-border rounded-xl px-4 py-3 text-primary text-lg font-mono uppercase" />
        </div>
      </div>
      
      <div className="flex-grow min-h-[200px] flex flex-col md:flex-row rounded-xl overflow-hidden shadow-inner border border-border">
        {shades.length > 0 ? shades.map((hex, i) => (
          ...
          >
             <span className="text-xs font-mono font-medium opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-primary p-1 rounded inline-block text-center mt-auto md:w-full select-all">
               {hex}
             </span>
          </div>
        )) : (
