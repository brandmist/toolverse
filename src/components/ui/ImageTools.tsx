    ...
    <div className="flex flex-col md:flex-row gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      <div className="flex-1 flex flex-col gap-4">
        ...
        {base64 && (
          <div className="relative">
            <div className="bg-card border border-border p-2 rounded-xl flex items-center justify-center max-h-[300px] overflow-hidden border border-white/10">
              <img src={base64} alt="Preview" className="max-w-full max-h-[280px] object-contain rounded-lg" />
            </div>
            <Button variant="outline" size="sm" className="absolute top-2 right-2 border-white/10 hover:bg-white/20 bg-black/50 text-text-primary" onClick={clearImage}>
              Clear
            </Button>
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col h-[400px] md:h-auto">
        <div className="flex justify-between items-center mb-2">
           <span className="text-sm font-medium text-text-primary">Base64 String</span>
           {base64 && (
             <Button variant="ghost" size="sm" className="h-8 text-text-muted hover:text-text-primary" onClick={() => navigator.clipboard.writeText(base64)}>
        ...
        <textarea
          readOnly
          className="w-full flex-grow p-4 bg-surface border border-border border-white/10 rounded-xl resize-none text-success font-mono text-xs break-all"
          value={base64}
      ...
      <div className="flex-1 space-y-6">
        <div>
           <div className="flex justify-between mb-1">
             <span className="text-sm text-text-primary">Blob Color</span>
        ...
        <div>
          <div className="flex justify-between mb-1">
            <span className="text-sm text-text-primary">Complexity</span>
            <span className="text-sm text-text-muted">{complexity} points</span>
          </div>
        ...
        <Button className="w-full bg-card border border-border hover:bg-card-hover text-text-primary border-0" onClick={generateBlob}>
          🎲 Generate New Blob
        </Button>
        <div className="pt-2">
           <span className="text-sm text-text-primary mb-2 block">SVG Code</span>
           <textarea
             readOnly
             className="w-full h-28 p-4 bg-surface border border-border border-white/10 rounded-xl resize-none text-success font-mono text-xs break-all"
             value={blob}
           />
           <div className="grid grid-cols-2 gap-2 mt-2">
             <Button variant="outline" className="border-white/10 hover:bg-white/10 text-text-primary" onClick={() => navigator.clipboard.writeText(blob)}>
               Copy SVG Code
             </Button>
             <Button className="bg-card border border-border hover:bg-card-hover text-text-primary border-0" onClick={downloadSvg}>
               Download SVG
             </Button>
           </div>
        </div>
      </div>
      <div className="flex-[1.5] flex items-center justify-center p-8 bg-surface border border-border rounded-xl border border-white/5 min-h-[300px]">
         <div
    ...
    <div className="flex flex-col gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5">
            <h3 className="font-semibold text-text-primary mb-4">Resize Settings</h3>
            <div>
              <label className="text-sm text-text-primary block mb-1">Width (px)</label>
              <input type="number" value={width} onChange={e => handleWidthChange(parseInt(e.target.value) || 0)} className="w-full bg-card border border-border border-white/10 rounded-lg px-3 py-2 text-text-primary" />
            </div>
            <div>
              <label className="text-sm text-text-primary block mb-1">Height (px)</label>
              <input type="number" value={height} onChange={e => handleHeightChange(parseInt(e.target.value) || 0)} className="w-full bg-card border border-border border-white/10 rounded-lg px-3 py-2 text-text-primary" />
            </div>
            <label className="flex items-center space-x-2 text-text-primary cursor-pointer pt-2">
              <input type="checkbox" checked={maintainAspect} onChange={(e) => setMaintainAspect(e.target.checked)} className="w-4 h-4 rounded border-white/20 bg-card border border-border" />
              <span className="text-sm">Maintain Aspect Ratio</span>
            </label>
            <div className="pt-4 flex gap-2">
              <Button className="flex-1 bg-card border border-border hover:bg-card-hover text-text-primary" onClick={downloadResized}>
                <Download className="w-4 h-4 mr-2" /> Download
              </Button>
              <Button variant="outline" className="border-white/10 hover:bg-white/10 text-text-primary" onClick={clearImage}>
                Clear
              </Button>
            </div>
          </div>
          <div className="flex-[1.5] flex items-center justify-center p-4 bg-card border border-border rounded-xl border border-white/5 min-h-[300px] overflow-hidden">
    ...
    <div className="flex flex-col md:flex-row gap-6 h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      <div className="flex-[1.5] flex flex-col gap-4">
        {!imageSrc ? (
          ...
          <div className="relative">
            <img src={imageSrc} alt="Preview" className="w-full rounded-xl object-contain bg-card border border-border max-h-[400px]" />
            <Button variant="outline" size="sm" className="absolute top-2 right-2 border-white/10 hover:bg-white/20 bg-black/50 text-text-primary" onClick={clearImage}>
              Clear
            </Button>
          </div>
      ...
      <div className="flex-1 right-panel flex flex-col gap-3">
         <h3 className="font-semibold text-text-primary mb-2">Dominant Colors</h3>
         {colors.length > 0 ? (
           <div className="grid grid-cols-2 gap-3">
             {colors.map((color, idx) => (
               <div key={idx} className="flex flex-col bg-card border border-border border-white/5 rounded-xl overflow-hidden group cursor-pointer" onClick={() => { navigator.clipboard.writeText(color); }}>
                 <div className="h-16 w-full transition-transform group-hover:scale-105" style={{ backgroundColor: color }}></div>
                 <div className="p-3 flex justify-between items-center text-sm">
                   <span className="text-text-primary font-mono uppercase">{color}</span>
           ...
           <div className="flex items-center justify-center h-48 bg-surface border border-border rounded-xl border border-white/5 text-text-muted text-sm p-6 text-center">
      ...
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-white/5 max-w-[300px]">
             <h3 className="font-semibold text-text-primary mb-4">Crop Area (%)</h3>
             <div>
               <label className="text-sm text-text-primary block mb-1">X Offset: {cropBox.x}%</label>
             ...
             <div>
               <label className="text-sm text-text-primary block mb-1">Y Offset: {cropBox.y}%</label>
             ...
             <div>
               <label className="text-sm text-text-primary block mb-1">Width: {cropBox.width}%</label>
             ...
             <div>
               <label className="text-sm text-text-primary block mb-1">Height: {cropBox.height}%</label>
          ...
          <div className="flex-[2] flex items-center justify-center p-4 bg-card border border-border rounded-xl border border-white/5 min-h-[300px]">
