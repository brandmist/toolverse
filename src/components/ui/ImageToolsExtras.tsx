    ...
    <div className="flex flex-col gap-6 h-full bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      {!previewUrl ? (
        ...
        >
          ...
          <p className="text-primary font-medium mb-1">Click or drag & drop image file</p>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-border max-w-[320px] flex flex-col justify-between">
             <div className="space-y-4">
               <div>
                  <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Source Format</h4>
                  <div className="bg-white border border-border border-border rounded-xl px-3 py-2 text-sm text-primary flex items-center justify-between">
                    <span className="font-bold text-primary">{activeExt}</span>
                    <span className="text-xs text-muted">{file ? (file.size > 1024 * 1024 ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : `${(file.size / 1024).toFixed(1)} KB`) : ''}</span>
                  </div>
               </div>
               
               <div>
                  <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Target Format</h4>
                  <div className="grid grid-cols-3 gap-2">
                    {outputFormats.map(fmt => (
                      <button
                        key={fmt.value}
                        onClick={() => setOutputFormat(fmt.value)}
                        className={`py-2 rounded-xl text-xs font-bold transition-colors ${outputFormat === fmt.value ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted hover:text-primary hover:border-border'}`}
                      >
                        {fmt.label}
                      </button>
                    ))}
             ...
             <div className="pt-4 space-y-2">
               ...
               <div className="flex gap-2">
                 <Button className="flex-grow bg-white border border-border hover:bg-white-hover text-primary font-semibold text-sm" onClick={handleDownload} disabled={isProcessing}>
                   <Download className="w-4 h-4 mr-2" /> Save as {outputFormat.toUpperCase()}
                 </Button>
                 <Button variant="outline" className="border-border hover:bg-white/10 text-primary" onClick={clear}>
                   Clear
                 </Button>
               </div>
             </div>
          ...
          <div className="flex-[2] flex flex-col items-center justify-center p-4 bg-white border border-border rounded-2xl border border-border min-h-[350px] relative">
             ...
             {isProcessing && (
               <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center p-6 text-center">
                 ...
                 <p className="text-primary font-medium text-sm">{statusText || 'Processing image...'}</p>
               </div>
             )}
          </div>
        </div>
      ...
      {!imageSrc ? (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4 bg-surface border border-border p-4 rounded-xl border border-border max-w-[300px]">
             <h3 className="font-semibold text-primary mb-4">Filters & Transforms</h3>

             <div>
                <label className="text-sm text-primary block mb-2">Flip</label>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setFlipH(!flipH)} className={`py-2 rounded-xl text-sm border font-medium transition-colors ${flipH ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted'}`}>Flip H</button>
                  <button onClick={() => setFlipV(!flipV)} className={`py-2 rounded-xl text-sm border font-medium transition-colors ${flipV ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted'}`}>Flip V</button>
                </div>
             </div>

             <div>
                <label className="text-sm text-primary block mb-2">Rotate</label>
                <div className="grid grid-cols-4 gap-1">
                  {[0, 90, 180, 270].map(r => (
                    <button key={r} onClick={() => setRotation(r)} className={`py-1.5 rounded-xl text-xs border font-medium transition-colors ${rotation === r ? 'bg-white border border-border border-border text-primary' : 'bg-white border border-border border-border text-muted'}`}>{r}°</button>
                  ))}
                </div>
             </div>

             <div className="pt-2">
                <label className="text-sm text-primary block mb-2">Color Filter</label>
                <select value={filter} onChange={e => setFilter(e.target.value)} className="w-full bg-white border border-border border-border rounded-xl px-3 py-2 text-primary">
             ...
             <div className="pt-4 flex gap-2">
              ...
              <Button variant="outline" className="border-border hover:bg-white/10 text-primary" onClick={() => { clearImage(); setFilter('none'); setFlipH(false); setFlipV(false); setRotation(0); }}>
                Clear
              </Button>
            </div>
          </div>
...
export function AIToolPlaceholder({ name, description }: { name: string, description: string }) {
  return (
    <div className="flex flex-col h-full bg-white border border-border rounded-2xl p-8  shadow-lg shadow-sm items-center justify-center text-center">
       ...
       <div className="bg-white border border-border border-border rounded-xl p-6 text-left max-w-lg w-full">
         ...
         <div className="flex gap-4 opacity-50 pointer-events-none grayscale">
            <Button className="w-full bg-white border border-border text-primary">
               Upload Image
            </Button>
         </div>
       </div>
