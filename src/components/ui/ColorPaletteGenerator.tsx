...
export function ColorPaletteGenerator() {
  ...
  return (
    <div className="flex flex-col h-full bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-lg shadow-black/10">
      <div className="flex-grow flex flex-col sm:flex-row rounded-xl overflow-hidden mb-6 border border-white/10 min-h-[250px]">
         {colors.map((color, i) => (
            <div key={i} className="flex-1 flex flex-col items-center justify-end p-4 transition-all duration-300 hover:flex-[1.5] cursor-pointer group" style={{backgroundColor: color}}>
               <button 
                 ...
                 className="bg-black/40 backdrop-blur-sm text-text-primary px-3 py-1.5 rounded-lg text-sm font-mono hover:bg-black/60 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity translate-y-4 group-hover:translate-y-0"
               >
      ...
      <Button className="w-full bg-card border border-border hover:bg-card-hover text-text-primary border-0" size="lg" onClick={generateColors}>
