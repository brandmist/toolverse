...
export function ColorPaletteGenerator() {
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-border rounded-2xl p-6  shadow-lg shadow-sm">
      <div className="flex-grow flex flex-col sm:flex-row rounded-xl overflow-hidden mb-6 border border-border min-h-[250px]">
         {colors.map((color, i) => (
            <div key={i} className="flex-1 flex flex-col items-center justify-end p-4 transition-all duration-300 hover:flex-[1.5] cursor-pointer group" style={{backgroundColor: color}}>
               <button 
                 ...
                 className="bg-black/40 backdrop-blur-sm text-primary px-3 py-1.5 rounded-xl text-sm font-mono hover:bg-black/60 flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity translate-y-4 group-hover:translate-y-0"
               >
      ...
      <Button className="w-full bg-white border border-border hover:bg-white-hover text-primary border-0" size="lg" onClick={generateColors}>
